...
describe('AuthService - password reset', () => {
  ...
  it('requestPasswordReset returns token when email exists', async () => {
    ...
    mockUsersService.findOneByEmail.mockResolvedValue({ id: 1, email: dto.email });
  ...
  it('resetPassword should succeed with valid token', async () => {
    ...
    const user = { id: 1, password_reset_expires: new Date(Date.now() + 10000) };
    ...
    expect(mockUsersService.updatePasswordAndClearReset).toHaveBeenCalledWith(user.id, dto.newPassword);
  });

  it('resetPassword should throw on invalid token', async () => {
    const dto: ResetPasswordDto = { token: 'bad', newPassword: 'newPass123' };
    mockUsersService.findOneByResetToken.mockRejectedValue(new Error('not found'));
