...
describe('ConsumerApiKeyService', () => {
  ...
  it('createForUser should save new key and return cleartext key and record', async () => {
    ...
    // save returns DB record with string-based id (generated by bigint), so make
    // the mock consistent with entity typing and tests. Spread `x` first so
    // the returned `id` override is preserved.
    saveMock = jest.fn((x) => Promise.resolve({ ...(x as object), id: '10' }));
